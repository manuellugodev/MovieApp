package com.manuellugodev.movie.retrofit.utils

object Constans {


    const val REQUEST_GET_POPULAR_MOVIE_LIST = "movie/popular"
    const val REQUEST_GET_TOP_RATED_MOVIE_LIST = "movie/top_rated"
    const val REQUEST_GET_MOVIE_DETAIL_BY_ID = "movie/{movie_id}"
    const val REQUEST_GET_IMAGES_MOVIE_BY_ID="movie/{movie_id}/images"
    const val REQUEST_GET_MOVIE_BY_SEARCH = "search/movie"
    const val REQUEST_PARAM_API_KEY = "api_key"
    const val REQUEST_PARAM_MOVIE_ID = "movie_id"
    const val REQUEST_PARAM_REGION = "region"
    const val REQUEST_PARAM_QUERY = "query"

}